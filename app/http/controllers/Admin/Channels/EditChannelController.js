...
const EditChannelService = require("../../../../services/Admin/Channels/EditChannelService");

const editChannel = async (req, res) => {
  const { body, session } = req;
  const rules = EditChannelService.rules;
  ...
  try {
    ...
    if (editChannelResult.hasOwnProperty("error")) {
      const responseObject = new ResponseObject(
        ...
        editChannelResult.error.message
      );
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:editChannel()`,
      },
      req.log
    );
  }
