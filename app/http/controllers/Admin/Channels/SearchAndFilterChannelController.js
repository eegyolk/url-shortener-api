...
const SearchAndFilterChannelService = require("../../../../services/Admin/Channels/SearchAndFilterChannelService");

const searchChannel = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterChannelService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:searchChannel()`,
      },
      req.log
    );
  }
...
const filterChannel = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterChannelService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:filterChannel()`,
      },
      req.log
    );
  }
