...
const SearchAndFilterDomainService = require("../../../../services/Admin/Domains/SearchAndFilterDomainService");

const searchDomain = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterDomainService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:searchDomain()`,
      },
      req.log
    );
  }
...
const filterDomain = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterDomainService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:filterDomain()`,
      },
      req.log
    );
  }
