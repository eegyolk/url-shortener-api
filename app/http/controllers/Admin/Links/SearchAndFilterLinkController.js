...
const SearchAndFilterLinkService = require("../../../../services/Admin/Links/SearchAndFilterLinkService");

const searchLink = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterLinkService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:searchLink()`,
      },
      req.log
    );
  }
...
const filterLink = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterLinkService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:filterLink()`,
      },
      req.log
    );
  }
