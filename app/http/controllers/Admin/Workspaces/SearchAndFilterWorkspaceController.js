...
const SearchAndFilterWorkspaceService = require("../../../../services/Admin/Workspaces/SearchAndFilterWorkspaceService");

const searchWorkspace = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterWorkspaceService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:searchWorkspace()`,
      },
      req.log
    );
  }
...
const filterWorkspace = async (req, res) => {
  const { body, session } = req;
  const rules = SearchAndFilterWorkspaceService.rules;
  ...
  try {
    ...
    Logger.log(
      ...
      {
        err,
        msg: `Error occurred in ${path.basename(__filename)}:filterWorkspace()`,
      },
      req.log
    );
  }
